...
export function drawRotatedText(
  ...
  // outline color: either user-supplied or auto-computed
  ...
  let baseY = position[1] - (isOutline ? ((1/3 + pxRange * 0.05) * scale) : 0)

  for (const line of lines) {
    // alignment shift along text‐axis
    ...
    for (const ch of Array.from(line)) {
      ...
      // size & offset in screen‐px
      const cw = met.lbwh[2] * sizePx
      const chh= met.lbwh[3] * sizePx
      const ox = met.lbwh[0] * sizePx
      const oy = met.lbwh[1] * sizePx
