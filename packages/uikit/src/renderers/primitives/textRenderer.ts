...
export function drawRotatedText(
  ...
  gl.uniform4fv(shader.uniforms.outlineColor, isOutline ? outlineColor : [finalColor[0],finalColor[1],finalColor[2],0] as Float32List)
  ...
  for (const line of lines) {
    ...
    let ax=0, ay=0
    if (alignment===HorizontalAlignment.CENTER) ax = -Math.cos(rotation)*lw/2, ay = -Math.sin(rotation)*lw/2
    if (alignment===HorizontalAlignment.RIGHT)  ax = -Math.cos(rotation)*lw,   ay = -Math.sin(rotation)*lw
...
export function drawTextOffset(
  gl: WebGL2RenderingContext,
  { font, xy, str, scale=1, color=[1,0,0,1], rotation=0, outlineColor=null, isOutline=false, maxWidth=0, direction=OffsetDirection.Below }: TextOffsetConfig
): void {
  let [x,y]=xy
  const w = font.getTextWidth(str, scale)
  switch(direction) {
    case OffsetDirection.Below: y += font.fontMets!.ascender*scale*font.textHeight*gl.canvas.height; break
    case OffsetDirection.Above: y += font.fontMets!.descender*scale*font.textHeight*gl.canvas.height; break
    case OffsetDirection.LeftOf:  x -= w; break
    case OffsetDirection.RightOf: x += w; break
    case OffsetDirection.CenteredOn: y += (font.fontMets!.ascender+font.fontMets!.descender)/2*scale*font.textHeight*gl.canvas.height; break
  }
  drawRotatedText(gl, {font, xy:[x,y], str, scale, color, rotation, outlineColor, isOutline, maxWidth, alignment:HorizontalAlignment.CENTER})
}

/** Convenience draw methods */
export const drawText = (gl: WebGL2RenderingContext, cfg: RotatedTextConfig) => drawRotatedText(gl, {...cfg, rotation:0})
export const drawTextBelow    = (gl: WebGL2RenderingContext, cfg: TextOffsetConfig) => drawTextOffset(gl, {...cfg, direction:OffsetDirection.Below})
export const drawTextAbove    = (gl: WebGL2RenderingContext, cfg: TextOffsetConfig) => drawTextOffset(gl, {...cfg, direction:OffsetDirection.Above})
export const drawTextLeftOf  = (gl: WebGL2RenderingContext, cfg: TextOffsetConfig) => drawTextOffset(gl, {...cfg, direction:OffsetDirection.LeftOf})
export const drawTextRightOf = (gl: WebGL2RenderingContext, cfg: TextOffsetConfig) => drawTextOffset(gl, {...cfg, direction:OffsetDirection.RightOf})
export const drawTextCenteredOn = (gl: WebGL2RenderingContext, cfg: TextOffsetConfig) => drawTextOffset(gl, {...cfg, direction:OffsetDirection.CenteredOn})
